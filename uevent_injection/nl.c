...
extern int netlink_open(struct nl_handler *handler, int protocol, int groups)
{
	socklen_t socklen;
	int sndbuf = 32768;
	int rcvbuf = 32768;
	...
	handler->fd = socket(AF_NETLINK, SOCK_DGRAM, protocol);
	if (handler->fd < 0)
		return -errno;
	...
	handler->local.nl_groups = groups;
